...
class AuthClient:
    ...
    async def register_on_auth(cls, password, password_confirmation, email) -> RegisterAuthResult:
        obj = {"password": password, "password_confirmation": password_confirmation, "email": email}
        full_url = f"{settings.auth_protocol_host_port}{settings.auth_register_url}"
        async with aiohttp.ClientSession() as session:
            async with session.post(
                full_url, headers={"Content-Type": "application/json"}, data=json.dumps(obj)
            ) as response:
    ...
    async def change_email(cls, access_token, email):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(full_url, headers=headers, json=obj) as response:
                json_obj = await response.json()
                if 200 <= response.status < 300:
    ...
    async def unregister(cls, access_token) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.delete(
                full_url, headers={"Content-Type": "application/json", "Authorization": f"Bearer {access_token}"}
            ) as response:
                if 200 <= response.status < 300:
                    return True
                else:
